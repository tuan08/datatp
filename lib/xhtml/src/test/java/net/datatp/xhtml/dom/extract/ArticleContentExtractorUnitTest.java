...
public class ArticleContentExtractorUnitTest {
  ...
  static URLVerifier VNEXPRESS = new URLVerifier(
      ...
      "http://vnexpress.net/gl/kinh-doanh/2011/09/nhung-bat-cap-trong-dieu-hanh-gia-xang-dau/",
  ...
  private void verifyAll(Fetcher fetcher) throws Exception {
    URLVerifier[] all = {
       VNEXPRESS, DANTRI, _24H, /*BEENET,*/ NGOISAONET, 
       BONGDA, /*VIETNAMNET,*/ DOCBAO, XALUAN, VIETBAO
    };
  ...
  public void test() throws Exception {
    ...
    //verifyAll(fetcher) ;
    VNEXPRESS.verify(fetcher, true) ;
    //BEENET.verify(fetcher, true) ;
    //DANTRI.verify(fetcher, true) ;
    //_24H.verify(fetcher, true) ;
    //BONGDA.verify(fetcher, true);
    //NGOISAONET.verify(fetcher, true) ;
    //VIETNAMNET.verify(fetcher, true);
    //XALUAN.verify(fetcher, true);
    //DOCBAO.verify(fetcher, true);
    VIETBAO.verify(fetcher, true) ;
  }
}