...
public class URLUnitTest {
  ...
  public void testURLNormalizer() throws Exception {
    ...
    URLParser normURL = new URLParser(url1) ;
    ...
    normURL = new URLParser(url7) ;
    Assert.assertEquals("/test/crawler", normURL.getPathWithParams()) ;
    Assert.assertEquals("http://test.moom.vn/test", normURL.getBaseURL()) ;

    String url8 = "http://anthinhi-c.com.vn?q=article/index.php" ;
    normURL = new URLParser(url8) ;
    Assert.assertEquals("anthinhi-c.com.vn", normURL.getHost()) ;

    String nowwwId = 
        new URLParser("http://clc.vn/?act=detail&icatid=99&id=183&module=product").getHostMD5Id() ;
    String wwwId = 
        new URLParser("http://www.clc.vn/?act=detail&icatid=99&id=183&module=product").getHostMD5Id() ;
    Assert.assertEquals(nowwwId, wwwId) ;
    
    URLParser dirAndNameTest1 = new URLParser("http://vietnamnet.vn/vn/giao-duc/");
  ...
  public void testURLParamClean() throws Exception {
    ...
    URLParser urlnorm= 
        new URLParser("http://www.thegioicauca.com/4rum/viewtopic.php?f=16&p=468&sid=086f5383d38cc1bc7643be3f7cfb1196&t=68") ;
    urlnorm.process(URL_PROCESSORS) ;
    Assert.assertNull(urlnorm.getParams().get("sid"));

    urlnorm= 
        new URLParser("http://forum.nguyentrai.de/viewtopic.php?f=5&sid=cc05fb6e4020b3bb983e68c126b170de&t=417440") ;
    urlnorm.process(URL_PROCESSORS) ;
    Assert.assertNull(urlnorm.getParams().get("sid"));

    urlnorm= 
        new URLParser("http://www.thegioicauca.com/4rum/viewtopic.php?f=16&p=468&s=086f5383d38cc1bc7643be3f7cfb1196&t=68") ;
    urlnorm.process(URL_PROCESSORS) ;
    Assert.assertNull(urlnorm.getParams().get("s"));

    urlnorm= 
        new URLParser("http://www.thegioicauca.com/4rum/viewtopic.php?NVS=086f5383d38cc1bc7643be3f7cfb1196&t=68") ;
    urlnorm.process(URL_PROCESSORS) ;
