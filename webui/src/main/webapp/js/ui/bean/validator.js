define([
  ...
  var validator = {
    ...
    validate: function(field, value) {
      if(!(field.validator && field.validator.name)) {
        return { success: true };
      }
      try {
        var validateMethod = this[field.validator.name] ;
        if (value instanceof Array) {
          for(var j = 0; j < value.length; j++) {
            validateMethod(field.validator, value[j]) ;
          }
        } else {
          validateMethod(field.validator, value) ;
        }
      } catch(err) {
